...
class ProductForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["category"].help_text = "Выберите категорию из выпадающего списка"

        self.fields["category"].widget.attrs.update({"class": "form-control", "id": "id_category"})
        ...
        self.fields["product_image"].help_text = "Поддерживаются форматы: JPEG, JPG, PNG. Максимальный размер: 5 МБ."
    ...
    def _check_forbidden_words(self, text, field_name):
        ...
        for word in self.FORBIDDEN_WORDS:
            if word in text_lower:
                raise ValidationError(f'В {field_name} нельзя использовать слово "{word}".')
