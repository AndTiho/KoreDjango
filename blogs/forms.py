...
class BlogForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["name"].widget.attrs.update(
            {"class": "form-control", "placeholder": "Введите заголовок ( не более 150 символов )"}
        )

        self.fields["text"].widget.attrs.update({"class": "form-control", "placeholder": "Введите содержание блога"})

        self.fields["published"].widget.attrs.update({"class": "form - check"})

        self.fields["image"].help_text = "Поддерживаются форматы: JPEG, JPG, PNG. Максимальный размер: 5 МБ."
